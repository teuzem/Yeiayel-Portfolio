...
export function SkillsChart({ skills, locale = "en" }: SkillsChartProps) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {Array.from(groupedSkills.entries()).map(([category, categorySkills]) => {
        ...
        const chartData = categorySkills.map((skill) => ({
          name: skill.name || dict.skills.unknown || "Unknown",
        ...
        // Calculate dynamic height based on number of skills (extra room
        // so long skill names never overlap, even with larger, bolder text)
        const chartHeight = Math.max(
          200,
          categorySkills.length * 46 + (categorySkills.length > 0 ? 32 : 0),
        );
              ...
              >
                ...
                >
                  ...
                  <YAxis
                    ...
                    axisLine={false}
                    width={180}
                    className="fill-foreground text-[11px] font-semibold leading-tight"
                  />
                  <ChartTooltip
                    cursor={false}
                    content={
                  ...
                  />
                  <Bar dataKey="proficiency" radius={[0, 6, 6, 0]} barSize={11}>
                    <LabelList
                      dataKey="proficiency"
                      position="right"
                      offset={4}
